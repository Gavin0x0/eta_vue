<template>
  ...
  <h3>先下载奖项模板，并按照要求录入数据「请勿随意更改模板内容」，完成后拖至下方进行上传</h3>
</el-card>
...
export default {
  ...
  methods: {
    ...
    handleDownload() {
      getAwardTemplate()
        .then((res) => {
          //closeDebug console.log("-----------下载奖项模板---------------");
          //closeDebug console.log(res);
          const blob = new Blob([res.data]);
          var downloadElement = document.createElement("a");
          var href = window.URL.createObjectURL(blob);
          downloadElement.href = href;
          downloadElement.download = decodeURIComponent("奖项导入模板.xls");
